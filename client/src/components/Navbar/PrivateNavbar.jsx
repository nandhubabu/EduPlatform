        ...
        <div className="w-full max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12 h-20 flex items-center justify-between gap-6 lg:gap-8">
          ...
          <div className="hidden lg:flex items-center gap-6 flex-shrink-0">
            ...
            <Link
              ...
              className="text-sm font-semibold text-slate-600 hover:text-slate-900 transition flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-100"
            >
              <FaThLarge className="text-indigo-500 text-sm" />
            ...
            {isInstructor && (
              <Link
                ...
                className="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3.5 py-2 rounded-xl hover:bg-emerald-100 transition flex items-center gap-2"
              >
                <FaPlusCircle />
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-3 p-1.5 pr-3 rounded-full hover:bg-slate-100 border border-slate-200 transition focus:outline-none"
              >
                <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-600 to-blue-600 flex items-center justify-center text-white text-sm font-black shadow-sm">
                  {userInitial}
                </div>
                <div className="text-left hidden xl:block">
                  <div className="text-xs font-bold text-slate-800 max-w-[100px] truncate">{username}</div>
                  <div className="text-[10px] text-indigo-600 font-semibold uppercase">{userProfile?.role || "Student"}</div>
                </div>
                <FaChevronDown className="text-slate-400 text-xs" />
              </button>

              {dropdownOpen && (
                <div className="absolute right-0 mt-3 w-72 bg-white border border-slate-200 rounded-2xl shadow-xl py-3 z-50 animate-fadeIn">
                  <div className="px-5 py-3 border-b border-slate-100">
                    <div className="flex items-center gap-3.5">
                      <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-indigo-600 to-blue-600 flex items-center justify-center text-white font-black text-lg">
                        {userInitial}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-slate-900 truncate">{username}</p>
                        <p className="text-xs text-slate-500 truncate">{userProfile?.email}</p>
                        <span className="inline-block mt-1 text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100">
                  ...
                  <div className="p-2 space-y-1">
                    <Link
                      to="/dashboard"
                      className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition"
                    >
                      <FaThLarge className="text-indigo-600 text-base" />
                      <div>
                        <div className="font-bold text-slate-900">My Learning Dashboard</div>
                        <div className="text-xs text-slate-500">Enrolled courses & progress</div>
                      </div>
                    </Link>
                    ...
                    {isInstructor && (
                      <Link
                        ...
                        className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition"
                      >
                        <FaLayerGroup className="text-purple-600 text-base" />
                        <div>
                          <div className="font-bold text-slate-900">Instructor Dashboard</div>
                          <div className="text-xs text-slate-500">Manage created courses</div>
                        </div>
                      </Link>
                    )}
                    ...
                    <Link
                      to="/settings"
                      className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition font-medium"
                    >
        ...
        {mobileOpen && (
          <div className="lg:hidden bg-white border-t border-slate-200 px-6 py-5 space-y-5 animate-fadeIn shadow-xl">
            <div className="flex items-center gap-3.5 pb-4 border-b border-slate-100">
              <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-indigo-600 to-blue-600 flex items-center justify-center text-white font-bold text-lg">
                {userInitial}
              </div>
              <div className="min-w-0">
                <p className="text-slate-900 font-bold truncate text-base">{username}</p>
                <p className="text-xs text-slate-500 truncate">{userProfile?.email}</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Link
                ...
                className="flex items-center gap-3.5 px-4 py-3 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-100"
              >
                <FaBookOpen className="text-blue-600 text-base" />
              ...
              {isInstructor && (
                <Link
                  to="/instructor-courses"
                  className="flex items-center gap-3.5 px-4 py-3 rounded-xl text-sm font-semibold text-emerald-700 hover:bg-emerald-50"
                >
                  <FaLayerGroup className="text-emerald-600 text-base" />
              ...
              <Link
                ...
                className="flex items-center gap-3.5 px-4 py-3 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-100"
              >
                <FaCog className="text-slate-500 text-base" />
                <span>Account Settings</span>
              </Link>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100">
              <button
                onClick={handleLogout}
                className="w-full flex items-center justify-center gap-2.5 py-3 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-100 text-sm font-bold transition border border-rose-100"
              >
                <FaSignOutAlt />
                <span>Sign Out</span>
              </button>
